...
/** Construye agregados (rawHourlyData, dailyData, monthlyData, etc.) desde paymentRecords para los meses indicados. Soporta historial completo. */
export const buildCloudwatchAggregates = action({
  args: {
  ...
  handler: async (ctx, { months }) => {
    ...
    for (const monthStr of months) {
      ...
      await ctx.runMutation(api.aggregatesCloudwatchMutations.clearAggregatesForMonth, {
        year,
        month,
      });
      ...
      for (let i = 0; i < rawHourlyRecords.length; i += BATCH_SIZE) {
        await ctx.runMutation(api.aggregatesCloudwatchMutations.batchInsertRawHourly, {
          records: rawHourlyRecords.slice(i, i + BATCH_SIZE),
        });
      }
      for (let i = 0; i < dailyDataRecords.length; i += BATCH_SIZE) {
        await ctx.runMutation(api.aggregatesCloudwatchMutations.batchInsertDailyData, {
          records: dailyDataRecords.slice(i, i + BATCH_SIZE),
        });
      }
      for (const r of monthlyRecords) {
        await ctx.runMutation(api.aggregatesCloudwatchMutations.batchInsertMonthlyData, {
          records: [r],
        });
      }
      for (let i = 0; i < hourlyDistRecords.length; i += BATCH_SIZE) {
        await ctx.runMutation(api.aggregatesCloudwatchMutations.batchInsertHourlyDistribution, {
          records: hourlyDistRecords.slice(i, i + BATCH_SIZE),
        });
      }
      for (let i = 0; i < dailyAmountRecords.length; i += BATCH_SIZE) {
        await ctx.runMutation(api.aggregatesCloudwatchMutations.batchInsertDailyAmountData, {
          records: dailyAmountRecords.slice(i, i + BATCH_SIZE),
        });
      }
      for (let i = 0; i < amountByMovementRecords.length; i += BATCH_SIZE) {
        await ctx.runMutation(api.aggregatesCloudwatchMutations.batchInsertAmountByMovement, {
