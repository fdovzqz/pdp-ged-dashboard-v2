# Patrón: Cargas masivas con Inngest (legacy)

**Nota:** Los pipelines de datamapping (full-history, clear, enrichment, backfill, load-from-date, fechaTransaccion, incremental) **ya no usan Inngest**. Orquestan con el [motor de pipeline en Convex](refactor-pipeline-convex.md); ver [mass-loads-pipeline-pattern.md](mass-loads-pipeline-pattern.md).

Este documento describe el **patrón histórico** con Inngest (un step por unidad, ejecución en paralelo, resultado en pipelineJobs). Se mantiene como referencia del diseño de unidades y resultados (completedMonths, failedMonths, byMonth).
