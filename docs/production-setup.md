...
4. **Build en Vercel**
   ...
   - En cada deploy a **Production**, `convex deploy`:
     ...
     - Pasa `NEXT_PUBLIC_CONVEX_URL` al `npm run build`, así la app queda apuntando a Convex prod.
...
| **Vercel → Build** | `npx convex deploy --cmd-url-env-var-name NEXT_PUBLIC_CONVEX_URL --cmd 'npm run build'` (ya en `vercel.json`). |
